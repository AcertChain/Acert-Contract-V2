...
function shouldBehaveLikeAsset20(
  errorPrefix,
  initialSupply,
  initialHolder,
  initialHolderId,
  ...
  describe('balanceOfCash', function () {
    describe('when the requested account has no tokens', function () {
      it('returns zero', async function () {
        expect(
          await this.token.methods['balanceOf(uint256)'](anotherAccountId),
    ...
    describe('when the requested account has some tokens', function () {
      it('returns the total amount of tokens', async function () {
        expect(
          await this.token.methods['balanceOf(uint256)'](initialHolderId),
    ...
    describe('when the token owner is not the zero id', function () {
      ...
      describe('when the recipientId is not the zero id', function () {
        ...
        describe('when the spender has enough allowance', function () {
          beforeEach(async function () {
            await this.token.methods['approve(uint256,address,uint256)'](
              initialHolderId,
              spenderAddr,
              initialSupply,
              {
          ...
          describe('when the token owner has enough balance', function () {
            ...
            it('transfers the requested amount', async function () {
              ...
              expect(
                await this.token.methods['balanceOf(uint256)'](tokenOwner),
              ).to.be.bignumber.equal('0');

              expect(
                await this.token.methods['balanceOf(uint256)'](to),
            ...
            it('decreases the spender allowance', async function () {
              ...
              expect(
                await this.token.methods['allowance(uint256,address)'](
                  tokenOwner,
                  spenderAddr,
                ),
              ).to.be.bignumber.equal('0');
            ...
            it('emits an approval by id event', async function () {
              expectEvent(
                ...
                'AssetApproval',
                {
                  owner: tokenOwner,
                  spender: spenderAddr,
                  value: await this.token.methods['allowance(uint256,address)'](
                    tokenOwner,
                    spenderAddr,
                  ),
                  isBWO: false,
          ...
          describe('when the token owner does not have enough balance', function () {
            ...
            it('reverts', async function () {
              await expectRevert(
                this.token.methods['transferFrom(uint256,uint256,uint256)'](
                  tokenOwner,
                  to,
                  amount,
                  {
      ...
      describe('when the recipientId is the zero Id', function () {
        ...
        it('burn token', async function () {
          const beforeBalance = await this.token.methods['balanceOf(uint256)'](
            tokenOwner,
          );
          ...
          expect(
            await this.token.methods['balanceOf(uint256)'](tokenOwner),
          ).to.be.bignumber.equal(beforeBalance.sub(amount));
        });
      });
  ...
  describe('isTrust safe conract and trust world', function () {
    ...
    shouldBehaveLikeAsset20IsTrust(
      initialSupply,
      initialHolder,
      initialHolderId,
      recipient,
...
function shouldBehaveLikeAsset20IsTrust(
  initialSupply,
  initialHolder,
  initialHolderId,
  spenderAddr,
...
function shouldBehaveLikeAsset20Transfer(
  errorPrefix,
  fromAddr,
  from,
  to,
  ...
  describe('when the recipientId is not the zero id', function () {
    describe('when the sender does not have enough balance', function () {
      ...
      it('reverts', async function () {
        await expectRevert(
          transfer.call(this, fromAddr, from, to, amount),
          `${errorPrefix}: transfer amount exceeds balance`,
        );
      });
    });
    ...
    describe('when the sender transfers all balance', function () {
      ...
      it('transfers the requested amount', async function () {
        ...
        expect(
          await this.token.methods['balanceOf(uint256)'](from),
        ).to.be.bignumber.equal('0');
    ...
    describe('when the sender is Freeze', function () {
      it('reverts', async function () {
        ...
        await expectRevert(
          transfer.call(this, fromAddr, from, to, balance),
  ...
  describe('when the recipientId is the zero Id', function () {
    it('burn token', async function () {
      ...
      expect(
        await this.token.methods['balanceOf(uint256)'](from),
      ).to.be.bignumber.equal(beforeBalance.sub(balance));
    });
  });
}

function shouldBehaveLikeAsset20Approve(
  errorPrefix,
  ownerAddr,
  owner,
  spenderAddr,
  ...
  describe('when the spender is not the zero Id', function () {
    ...
    describe('when the owner is frozen', function () {
      it('reverts', async function () {
        ...
        await expectRevert(
          approve.call(this, ownerAddr, owner, spenderAddr, supply),
  ...
  describe('when the spender is the zero address', function () {
    it('reverts', async function () {
      await expectRevert(
        approve.call(this, ownerAddr, owner, ZERO_ADDRESS, supply),
