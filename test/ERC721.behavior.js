  ...
  context('with minted tokens', function () {
    ...
    describe('balanceOf', function () {
      ...
      context('when the given address does not own any tokens', function () {
        it('returns 0', async function () {
          expect(
            await this.token.methods['balanceOf(address)'](other),
      ...
      context('when querying the zero address', function () {
        it('throws', async function () {
          await expectRevert(
            this.token.methods['balanceOf(address)'](ZERO_ADDRESS),
            'Asset721: address zero is not a valid owner',
          );
        });
      });
    });
    ...
    describe('transfers', function () {
      ...
      beforeEach(async function () {
        ...
        await this.token.methods['setApprovalForAll(address,bool)'](
          operator,
          true,
          {
            from: owner,
      ...
      describe('via transferFrom', function () {
        shouldTransferTokensByUsers(function (from, to, tokenId, opts) {
          return this.token.methods['transferFrom(address,address,uint256)'](
            from,
            to,
            tokenId,
            opts,
    ...
    describe('safe mint', function () {
      ...
      describe('via safeMint', function () {
        ...
        it('calls onERC721Received — with data', async function () {
          ...
          const receiverId = await this.Metaverse.getAccountIdByAddress(
            this.receiver.address,
          );

          const receipt = await this.token.safeMint(receiverId, tokenId, data);
        ...
        it('calls onERC721Received — without data', async function () {
          ...
          const receipt = await this.token.safeMint(receiverId, tokenId, '0x');
    ...
    describe('setApprovalForAll', function () {
      ...
      context('when the operator is the owner', function () {
        it('reverts', async function () {
          await expectRevert(
            this.token.methods['setApprovalForAll(address,bool)'](owner, true, {
              from: owner,
            }),
            'Asset721: approval to current account',
          );
