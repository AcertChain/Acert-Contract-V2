...
function shouldBehaveLikeAsset20ProxyBWO(
  errorPrefix,
  initialSupply,
  initialHolder,
  initialHolderId,
  ...
  describe('transferCashBWO proxy and approveCashBWO proxy', function () {
    ...
    it('addAuthAddressBWO', async function () {
      await this.Metaverse.removeAuthAddress(initialHolderId, authAccount, {
        from: initialHolder,
      });
      ...
      const authSignature = await this.authAccountSinger._signTypedData(
        this.domain,
        this.signAuthTypes,
        value,
      );
      ...
      const signature = await this.initialHolderSinger._signTypedData(
        this.domain,
        BWOType,
        BWOvalue,
      );

      await this.Metaverse.addAuthAddressBWO(
        initialHolderId,
        authAccount,
        initialHolder,
        deadline,
    ...
    it('approveAssetBWO proxy', async function () {
      ...
      const signature = await this.authAccountSinger._signTypedData(
        ...
        BWOType,
        {
          ownerId: initialHolderId.toString(),
          spender: recipient,
